...
contract ZABU is ERC20('ZABU Token', 'ZABU') {
...
// MasterChef is the master of STROM. He can make zabu and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once zabu is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accZabuPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accZabuPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accZabuPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    // The Zabu TOKEN!
    ZABU public immutable Zabu;
    address public devAddress;
	address public treasuryAddress;
    address public feeAddress;
    

    // Zabu tokens created per block.
    uint256 public ZabuPerBlock = 500 ether;
    
    uint256 constant MAX_ZABU_PERBLOCK= 500 ether;
    ...
    // The block number when Zabu mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ZabuPerBlock);
    ...
    // Update the given pool's Zabu allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending Zabues on frontend.
    function pendingZabu(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            accZabuPerShare = accZabuPerShare.add(ZabuReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accZabuPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ZabuReward = multiplier.mul(ZabuPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Zabu.mint(devAddress, ZabuReward.mul(15).div(100));
		Zabu.mint(treasuryAddress, ZabuReward.mul(10).div(100));
        Zabu.mint(address(this), ZabuReward);
        // Automatically burn 2% of minted tokens
        Zabu.mint(BURN_ADDRESS, ZabuReward.mul(20).div(1000));
        pool.accZabuPerShare = pool.accZabuPerShare.add(ZabuReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Zabu allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accZabuPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeZabuTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accZabuPerShare).div(1e18);
    ...
    // Safe Zabu transfer function, just in case if rounding error causes pool to not have enough Zabu.
    function safeZabuTransfer(address _to, uint256 _amount) internal {
        uint256 ZabuBal = Zabu.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ZabuBal) {
            transferSuccess = Zabu.transfer(_to, ZabuBal);
        } else {
            transferSuccess = Zabu.transfer(_to, _amount);
        }
        require(transferSuccess, "safeZabuTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ZabuPerBlock) external onlyOwner {
        require(_ZabuPerBlock<=MAX_ZABU_PERBLOCK,"Too high");
        massUpdatePools();
        ZabuPerBlock = _ZabuPerBlock;
        emit UpdateEmissionRate(msg.sender, _ZabuPerBlock);
    }
