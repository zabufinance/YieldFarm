...
contract ZABUFarm is Ownable {
    ...
    // The Defily Reward TOKEN.
    IERC20 public zabuToken;
    // Dev address.
    ...
    function safeTokenTransferReward(uint256 reward) internal {
        uint256 devReward = reward.div(100).mul(15); // 15%
        uint256 communityReward = reward.div(10); // 10%
        uint256 totalReward = reward.add(devReward).add(communityReward);
        uint256 rewardBal = zabuToken.balanceOf(address(this));

        require(rewardBal >= totalReward, "Balance is not enough");

        zabuToken.transfer(devAddr, devReward);
        zabuToken.transfer(communityAddr, communityReward);
        zabuToken.transfer(address(this), reward);
    }
